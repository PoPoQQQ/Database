        ...
        if(this->size() < minSize) {
            ...
            if(sibling != nullptr) {
                ...
                if(sibling->size() > minSize) {
                    ...
                    this->parent->removeBetween(sibling, this);
                    BplusLeaf<K, T>* tmp = this->tree->leafHead;
                    if(tmp == sibling) {
                        this->tree->leafHead = this;
                    } else {
                        while(tmp != NULL && tmp->next != sibling){
                            tmp = tmp->next;
                        }
                        if(tmp != NULL) tmp->next = this;
                    }
                    BplusNode<K, T>::debug[sibling->no - 1] = NULL;
                    delete sibling;
                }
                ...
                if(sibling->size() > minSize) {
                    ...
                    BplusLeaf<K, T>* tmp = this->tree->leafHead;
                    if(tmp == this) {
                        this->tree->leafHead = sibling;
                    } else {
                        while(tmp != NULL && tmp->next != this){
                            tmp = tmp->next;
                        }
                        if(tmp != NULL) tmp->next = sibling;
                    }
                    BplusNode<K, T>::debug[this->no - 1] = NULL;
                    delete this;
                }
...
public:
    BplusTree(int maxSize) : maxSize(maxSize){
        ...
        this->leafHead = static_cast<BplusLeaf<K, T>*>(this->root);
    }
    ~BplusTree() {
        if(this->root != NULL)
            delete this->root;
